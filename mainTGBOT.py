...
async def show_exercise_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not user_data:
        await callback.message.answer("Ошибка: данные не найдены. Начните заново.")
        return
...
async def cmd_help(message: types.Message):
    help_text = (
        "📋 **ПОМОЩЬ ПО БОТУ:**\n\n"
        "Бот предназначен для помощи в работе по профессиональной служебной и физической подготовки сотрудников.\n\n"
